...
Generate a brief response (1-2 sentences) introducing these options. For each block, provide a very short (10 words max) geometrical description."""
                    ...
                    for i, c in enumerate(candidates, 1):
                        block_material = c.get('material', 'N/A')
                        print(f"  {i}. {c['name']} [{block_material}]")
                    print()
...
Be friendly and use simple language. Keep it concise but informative."""
            ...
            if not user_input and default != "":
                ...
                # Try to extract numeric value with LLM understanding
                try:
                    numbers = re.findall(r'[-+]?\d*\.?\d+', user_input)
                    if numbers:
                        value = float(numbers[0])
                    else:
                        # Maybe user said "default" or similar
                        if any(word in user_input.lower() for word in ["default", "same", "ok", "fine"]):
                            if default != "":
                                value = default
                                print(f"   (Using default: {default})")
                            else:
                                print("\n🏛️ Athena: This parameter has no default. Please enter a number.")
                                continue
                        else:
                            print("\n🏛️ Athena: Please enter a number, or press Enter for default.")
                            continue
                except:
                    if default != "":
                        value = default
        ...
        # ========================================
        # PHASE: GENERATING CODE (with file explorer link)
        # ========================================
